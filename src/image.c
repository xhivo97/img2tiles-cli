...
static const wchar_t *valid_extensions[] = { L".png", L".PNG", L".jpg", L".JPG",
	L".jpeg", L".JPEG" };
#else
static const char *valid_extensions[] = { ".png", ".PNG", ".jpg", ".JPG",
	".jpeg", ".JPEG" };
#endif
		...
		if (img->png_ptr != NULL) {
			...
			if (img->info_ptr != NULL)
				png_destroy_read_struct(
					&img->png_ptr, &img->info_ptr, NULL);
			else
				png_destroy_read_struct(
					&img->png_ptr, NULL, NULL);
		}
...
{
	img->pad_size =
		get_padded_size(img->width, img->height, img->tile_size);
...
int image_init_read_png(struct image *img)
{
	...
	for (int i = 0; i < img->tile_size; i++) {
		img->row_pointers[i] =
			malloc(sizeof(png_byte) * img->width * 4);
...
int image_init_read_jpeg(struct image *img)
{
	...
	img->row_buf = malloc(sizeof(char) * img->cinfo.output_height *
			      img->cinfo.output_width * 3);

	DEBUG_PRINT("[JPEG] width: %d, height %d\n", img->cinfo.output_width,
		img->cinfo.output_height);
	return EXIT_SUCCESS;
}
